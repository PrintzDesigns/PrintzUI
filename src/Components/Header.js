...
class Header extends Component {
  ...
  render() {
    ...
    const dropdown_notAuthenticated = isAlreadyAuthenticated ? '' : (
      <div className="dropdown">
        <ButtonDropdown isOpen={this.state.dropdownOpen} toggle={this.toggle}>
          <DropdownToggle caret>
          </DropdownToggle>
          <DropdownMenu>
            <DropdownItem header>Sign In</DropdownItem>
            <DropdownItem header>Invite Friends</DropdownItem>
          </DropdownMenu>
        </ButtonDropdown>
      </div>
    )
    ...
    const dropdown  = isAlreadyAuthenticated ? (
      <div className="drop-down">
        <ButtonDropdown isOpen={this.state.dropdownOpen} toggle={this.toggle}>
          <img alt="" className="hamburger-nosignin-1" src={hamburger} />
          <DropdownMenu>
